...
- **`eksctl create iamserviceaccount` doesn't survive the disposable-cluster cycle.** It tracks whether it has work to do via a CloudFormation stack name (`eksctl-<cluster>-addon-iamserviceaccount-<ns>-<name>`), not the live cluster state. Since `terraform destroy` never touches that stack, the next `terraform apply` gets a brand-new cluster with a brand-new OIDC provider, but eksctl finds the old stack "already exists" and skips real work - leaving an IAM trust policy pinned to a now-deleted OIDC provider ID and, the first time this happened, no Kubernetes ServiceAccount at all. Symptom chain: LB controller pods stuck at `0/2` (`serviceaccount not found`), then after a manual SA fix, `AssumeRoleWithWebIdentity` 403s and the ALB never gets created. `bootstrap-eks.sh` compared the stack's trust policy against the cluster's current OIDC issuer before calling eksctl as a workaround - superseded now by the fix below, which makes the whole bug class impossible instead of detecting it after the fact.
- **Fixed for good by moving IRSA into Terraform itself.** `eks/modules/eks/irsa.tf` creates the OIDC provider (`aws_iam_openid_connect_provider`) and the LB Controller's IAM role in the *same Terraform state* as the cluster. Destroy always removes both together with the cluster; the next apply always recreates both together, always matched - the eksctl/CloudFormation staleness class of bug simply can't happen when there's no separate, longer-lived resource to go stale.
